...
class Heap{
    ...
    void heapSort();//sorts the array in descending order
    void reHeapify();//rebuild the heap (min-heap)
    private:
    void upHeap(int anIndex);
    void downHeap(int anIndex);
    void downHeap1(int anIndex);//different implementation
    void downHeap2(int anIndex, int end);//partial downHeap
    void inOrderTraversal(int anIndex);
    
    Node* _heap;
    int _heapSize;//array size
    int _numElements;//number of data points in heap (array)
    int _ROOT_INDEX;
};
